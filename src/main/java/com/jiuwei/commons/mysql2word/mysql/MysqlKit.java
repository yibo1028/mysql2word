...
public class MysqlKit extends MysqlConfig {
	...
	private static void initTableInfos() {
		...
		try {
			...
			while (tableResultSet.next()) {
				System.out.print("目录名：" + tableResultSet.getString(1));
				System.out.print(" 模式名：" + tableResultSet.getString(2));
				System.out.print(" 表名：" + tableResultSet.getString(3));
				System.out.print(" 表的类型：" + tableResultSet.getString(4));
					...
					while (rs.next()) {
						for (String fieldName : tableRelation.keySet()) {
							...
							if (!StringUtils.isNullOrEmpty(fieldValue) && fieldValue.equals("id")) {
								...
								mysqlTable.setTitle(mysqlTable.getTitle() + "(" +REMARKS+")");
							}
	...
	public static void addIndexInfo() throws Exception {
		...
		for (MysqlTable mysqlTable : tableList) {
			rs = dbmd.getIndexInfo(null, null, mysqlTable.getTableName(), false, false);
			for (String fieldName : tableIndex.keySet()) {
				List<String[]> fieldList = new ArrayList<String[]>(mysqlTable.getFieldList().size());
